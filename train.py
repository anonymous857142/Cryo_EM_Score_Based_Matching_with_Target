...
def main(args: argparse.Namespace):
    ...
    log_file = os.path.join(
        log_dir,
        f"{dataset_tag}_{cfg.running_config.model_type}_train_TSM_10A+3A.log"
    )
    ...
    if args.exp_name == "test":
        ...
        save_dir = os.path.join(test_root, f"{dataset_tag}_test_outputs_TSM_10A+3A")
        ...
        save_dir = os.path.join(compare_root, f"feature_maps_{dataset_tag}_TSM_10A+3A")
        os.makedirs(save_dir, exist_ok=True)
        ...
        for batch in train_dataloader:
            ...
            if global_step % save_interval == 0:
                ...
                visualization_path = os.path.join(
                    train_vis_dir,
                    f"{dataset_tag}_{category_tag}_10A+3A",
                    f"step_{global_step}"
                )
                ...
                if metrics['ssim'].mean()>best_ssim:
                    ...
                    checkpoint_path = os.path.join(train_ckpt_dir, f"{dataset_tag}_best_model_TSM_10A+3A.pt")
    ...
    final_path = os.path.join(train_ckpt_dir, f"{dataset_tag}_TSM_10A+3A.pt")
    plot_save_path = os.path.join(train_loss_dir, f"{dataset_tag}_loss_TSM_10A+3A.png")
    sim_path = os.path.join(train_sim_dir, f"{dataset_tag}_sim_TSM_10A+3A.png")
    plot_loss_curves(
        train_loss_history,
        val_loss_history,
        train_dsm_history,
